...
top to bottom. There is no package, no `setup.py`, no CI.

Two jobs, deliberately kept apart:

- **Reading.** `gpt_nano.py`, `qwen_nano.py`, `deepseek_nano.py`,
  `qwen_next_nano.py` each teach one architecture end to end.
- **Experimenting.** `attention_zoo.py` plus the component flags on
  `qwen_next_nano.py` let you swap pieces without touching the teaching files.
...
```bash
python attention_zoo.py                  # all attention variants (~1 min)
python qwen_next_nano.py --self-check     # hybrid components, init properties
python qwen_next_nano.py --train-check    # what only gradients reveal (~30s)
python deepseek_nano.py --self-check      # MoE routing and load balancing
```
...
Registering picks it up everywhere: `gpt_nano.py --attention myattn`,
`--attention all` benchmarks it, and `python attention_zoo.py` tests it for
...
Components in `qwen_next_nano.py` follow a consistent shape:
...
- **Single-file models.** `qwen_next_nano.py` imports the training loop from
  `qwen_nano.py` rather than copying it, but its *architecture* is all local.
...
- No component is verified to *improve* anything. A 3.5M model on a tiny corpus
  ...
  as results.
- `ARCHITECTURES.md` documents every component, its paper, and the
...
- **Do not delete the gotcha notes** in docstrings or `ARCHITECTURES.md` § 6.
