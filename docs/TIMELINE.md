...
| 2026-08 | 3:1 GDN + Qwen Sparse Attention (4-token micro-block indexer, top-512 blocks), 51B n-gram embedding at layer 2, 512-expert MoE top-10, Gated Residual (4 branches, no mixing matrix) | Qwen3.8-Flash-Next, released 2026-08; arXiv 2608.30320 | `--attention qsa`, `--attn qsa` in the hybrid, `--engram-dim`, `--residual gr`; preset `configs/qwen_flash_next.json` |
| 2026-09 | Causal encoder-decoder (20+20 layers, decoders read K/V from encoder layers), hierarchical sparse indexer (full / reindex / reuse), Engram at two layers, FP4 KV cache, DSpark draft head | DeepSeek-V4.1-Flash, released 2026-09-10 | `--kv-sources`, `--index-modes full,reindex,reuse --candidate-top-k`, `--engram-dim`; preset `configs/deepseek_v41_ced.json`; DSpark not here |
